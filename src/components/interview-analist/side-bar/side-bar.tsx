  ...
  useEffect(() => {
    const fetchNavItems = async () => {
      try {
        ...
        const response = await axios.post("/api/interview-analyzer/interview-analyzer-sidebar");
  ...
  return (
    <aside
      className={cn(
        ...
        "hidden md:block md:w-[150px] lg:w-[200px] p-4"
      )}
    >
      <nav className="flex flex-col space-y-3 mt-[80px]">
        {loading ? (
          ...
          navItems.map(({ companyname, slug }) => {
            const isActive = pathname === `/interview-analyzer/${slug}`;
            return (
              <Link
                key={slug}
                href={`/interview-analyzer/${slug}`}
                className={cn(
